...
# (Optional) hooks – quality gates before plan/apply for all children
before_hook "fmt" {
	commands = ["plan", "apply"]
	execute  = ["bash", "-lc", "terraform fmt -recursive -check"]
}
before_hook "validate" {
	commands = ["plan"]
	execute  = ["bash", "-lc", "terraform validate"]
}
before_hook "tflint" {
	commands = ["plan"]
	execute  = ["bash", "-lc", "tflint --no-color || true"]
}
before_hook "checkov" {
	commands = ["plan"]
	execute  = ["bash", "-lc", "checkov -d . || true"]
}
